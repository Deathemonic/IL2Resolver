...
public static class ICallAnalyzer
{
    ...
    public static bool IsICall(MethodDef methodDef)
    {
        ...
        {
            if (attr.TypeFullName is "UnityEngine.Bindings.FreeFunctionAttribute" or "UnityEngine.Bindings.NativeMethodAttribute"
    ...
    public static bool IsExternMethod(MethodDef methodDef)
    {
        ...
        foreach (var attr in methodDef.CustomAttributes)
        {
            if (attr.TypeFullName != "System.Runtime.CompilerServices.MethodImplAttribute") continue;
            ...
            if (attr.ConstructorArguments.Count > 0)
            {
                ...
                if (arg.Value is int intValue && intValue == 4096)
                    return true;
                if (arg.Value is short shortValue && shortValue == 4096)
                    return true;
            }
        }
    ...
    public static bool IsSimpleWrapper(MethodDef methodDef)
    {
        ...
        foreach (var instr in methodDef.Body.Instructions)
        {
            ...
            var isSimple = code is Code.Nop or Code.Ret
                or Code.Ldarg_0 or Code.Ldarg_1 or Code.Ldarg_2 or Code.Ldarg_3
                or Code.Ldarg_S or Code.Ldarg
                or Code.Ldarga_S or Code.Ldarga
                or Code.Call or Code.Callvirt
                ...
                or Code.Ldc_I4_M1 or Code.Ldc_I4_S or Code.Ldc_I4
                or Code.Ldc_I8 or Code.Ldc_R4 or Code.Ldc_R8
                or Code.Ldnull
                or Code.Box or Code.Unbox or Code.Unbox_Any
                or Code.Castclass or Code.Isinst
                or Code.Ldflda or Code.Ldfld;
    ...
    public static MethodDef? FindTargetICall(MethodDef methodDef) => 
        FindTargetICallInternal(methodDef, []);
    ...
    public static bool HasMatchingParams(MethodDef wrapper, MethodDef icall)
    {
        ...
        if (wrapper.IsStatic == icall.IsStatic)
        {
            ...
            {
                if (wrapperParams[i].Type.FullName != icallParams[i].Type.FullName)
                    return false;
            }
        }
        else if (!wrapper.IsStatic && icall.IsStatic && icallParams.Count > 0)
        {
            ...
            for (var i = 0; i < wrapperParams.Count; i++)
            {
                if (wrapperParams[i].Type.FullName != icallParams[i + 1].Type.FullName)
                    return false;
            }
        }
        else
    ...
    public static (string? Name, List<string>? Args) GetWrappedICallInfo(MethodDef methodDef)
    {
        ...
        if (calledMethod.FullName == methodDef.FullName)
            return (null, null);

        if (methodDef.IsStatic != calledMethod.IsStatic)
            return (null, null);
        ...
        if (isRegularICallWrapper && calledMethod.Name.String == methodDef.Name.String)
            return (null, null);
    ...
    public static StaticDelegateInfo? GetStaticDelegateInfo(MethodDef methodDef)
    {
        ...
        foreach (var instr in instructions)
        {
            ...
            if (instr.OpCode.Code == Code.Ldflda && instr.Operand is IField fieldRef)
            {
    ...
    private static bool IsFloatType(TypeSig typeSig) =>
        typeSig.FullName is "System.Single" or "System.Double";
    ...
    private static bool AreTypesCompatible(TypeSig wrapperType, TypeSig targetType)
    {
        ...
        if (wrapperType is GenericInstSig || targetType is GenericInstSig)
            return wrapperType.FullName == targetType.FullName;
        ...
        if (wrapperDef.IsClass && targetDef.FullName == "System.Object")
            return true;
